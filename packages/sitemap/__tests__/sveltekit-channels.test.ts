...
describe('SvelteKit route channels', () => {
	...
	it('regenerates an existing channel tree without leaving stale files', async () => {
		const { sourceRoutesRoot, generatedRoutesRoot } = await createTempRoutes()
		const result = await generateSvelteKitRouteChannel({
			sourceRoutesRoot,
			generatedRoutesRoot,
		...
		await writeFixture(result.targetRoot, 'stale/+page.svelte', 'stale')
		...
		await expect(
			readFile(path.join(result.targetRoot, 'stale/+page.svelte'), 'utf8')
		).rejects.toThrow()
		await expect(readFile(path.join(result.targetRoot, 'demo/+page.svelte'), 'utf8')).resolves.toBe(
			'demo'
		)
	})
