...
describe('scanSvelteKitRoutes', () => {
	...
	it('normalizes routing wrappers before dynamic detection and sibling matching', () => {
		const stripOptionalLanguage = (path: string) =>
			path.replace(/^\/\[\[lang=lang\]\]/, '') || '/'
		const entries = scanSvelteKitRoutes(
			mockGlob([
				'/src/routes/[[lang=lang]]/+page.svelte',
				'/src/routes/[[lang=lang]]/about/+page.svelte',
				'/src/routes/[[lang=lang]]/blog/[slug]/+page.svelte'
			]),
			{
				normalizePath: stripOptionalLanguage,
				serverGlob: mockGlob(['/src/routes/[[lang=lang]]/about/+page.server.ts'])
			}
		)

		expect(entries.map((entry) => entry.path)).toEqual(['/', '/about'])
		expect(entries.find((entry) => entry.path === '/about')?.hasServerLoad).toBe(true)
	})
