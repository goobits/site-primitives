...
export async function generateSvelteKitRouteChannel({
	...
	for (const entry of entries) {
		if (!isRouteFile(entry.relativePath, routeFileSet)) {
			if (await copyStaticRouteFile(entry, targetRoot, copiedPrivateDirectories)) {
				generatedRelativePaths.add(entry.relativePath)
			}
			continue
		}
	...
	for (const relativePath of await copyAssetsForIncludedRoutes(
		entries,
		includedDirectories,
		targetRoot,
		routeFileSet
	...
	for (const relativePath of await copyLayoutsForIncludedRoutes(
		entries,
		includedDirectories,
		targetRoot,
		sourceRoutesRoot,
	...
	await removeStaleGeneratedRouteFiles(targetRoot, generatedRelativePaths)
...
async function copyStaticRouteFile(
	...
	if (!firstSegment || !copiedPrivateDirectories.has(firstSegment)) return false
...
async function copyRouteFile(
	...
	try {
		...
		if (source.equals(existing)) return
	} catch {
		// Missing targets are created below; unreadable targets are replaced.
	}

	await mkdir(path.dirname(targetPath), { recursive: true })
	const tempPath = path.join(
		path.dirname(targetRoot),
		`.${path.basename(targetRoot)}.${process.pid}.${Date.now()}.${Math.random()
			.toString(36)
			.slice(2)}.tmp`
	)
		...
		while (current !== '..') {
			for (const layoutFile of layoutFiles) {
				if (path.dirname(layoutFile.relativePath) === current) {
					...
					copied.push(layoutFile.relativePath)
				}
			}
...
async function removeStaleGeneratedRouteFiles(
	targetRoot: string,
	generatedRelativePaths: Set<string>
): Promise<void> {
	await removeStaleGeneratedRouteFilesInDirectory(targetRoot, targetRoot, generatedRelativePaths)
}

async function removeStaleGeneratedRouteFilesInDirectory(
	targetRoot: string,
	directory: string,
	generatedRelativePaths: Set<string>
): Promise<boolean> {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			const keepDirectory = await removeStaleGeneratedRouteFilesInDirectory(
				targetRoot,
				absolutePath,
